...
def test_llm_with_tools():
    ...
    try:
        ...
        messages = [
            SystemMessage(content="You are a helpful assistant. You have access to search tools."),
            HumanMessage(content="What is the definition of 'photosynthesis'? You can use your knowledge.")
        ]
...
def test_tavily_search():
    ...
    try:
        ...
        start = time.time()
        result = web_search_tool.invoke({"query": "what is machine learning definition"})
        elapsed = time.time() - start
...
def test_rag_pipeline():
    ...
    if not (getattr(settings, 'SUPABASE_URL', '') and getattr(settings, 'SUPABASE_KEY', '')):
    ...
    try:
        ...
        print_result("Results", "empty (expected for non-existent deck)" if not result else f"{len(result)} chars")
...
def test_guardrail():
    ...
    try:
        ...
        print_result("Safe content check", f"is_safe={result.get('is_safe')}, time={elapsed:.2f}s")
...
def test_full_pipeline():
    ...
    try:
        ...
        return final_state.get("is_safe", False) and final_state.get("final_json", {}).get("back")
    except Exception as e:
        print_result("Error", str(e), False)
        import traceback
        traceback.print_exc()
...
def test_api_endpoint():
    ...
    try:
        ...
        start = time.time()
        response = client.post('/api/agent/flashcard/backside/rapid/', {
        ...
        start = time.time()
        response = client.post('/api/agent/flashcard/backside/', {
