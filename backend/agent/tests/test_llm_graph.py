...
class TestContextBuilderNode:
    ...
    def test_builds_context_for_valid_user_and_deck(self, test_user_with_profile, test_deck, base_state):
...
class TestGuardrailNode:
    ...
    def test_allows_safe_content(self, mock_llm, base_state):
        ...
        with patch('agent.llm_graph.JsonOutputParser') as mock_parser:
            mock_parser.return_value.invoke = lambda x: {"allowed": True, "reason": "Safe"}
            with patch('agent.llm_graph.ChatPromptTemplate') as mock_prompt:
                mock_chain = MagicMock()
                mock_chain.invoke.return_value = {"allowed": True, "reason": "Safe"}
                mock_prompt.from_messages.return_value.__or__ = lambda self, other: MagicMock(__or__=lambda self, other: mock_chain)
    ...
    def test_blocks_unsafe_content(self, mock_llm, base_state):
        ...
        with patch('agent.llm_graph.ChatPromptTemplate') as mock_prompt:
            mock_chain = MagicMock()
            mock_chain.invoke.return_value = {"allowed": False, "reason": "Inappropriate"}
            mock_prompt.from_messages.return_value.__or__ = lambda self, other: MagicMock(__or__=lambda self, other: mock_chain)
...
class TestGeneratorNode:
    ...
    def test_extracts_draft_from_last_message(self, base_state):
        ...
        base_state["messages"] = [HumanMessage(content="What is mitosis?"), mock_message]
...
class TestFormatterNode:
    ...
    def test_formats_final_json(self, base_state):
        ...
        base_state["generation_meta"] = {"rag_used": True, "features_used": ["examples"]}
...
class TestGraphCompilation:
    ...
    def test_graph_has_expected_nodes(self):
        ...
        expected_nodes = [
            "context_builder", "guardrail", "agent", 
            "tools", "generator", "critic", "formatter"
        ]
